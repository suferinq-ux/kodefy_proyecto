...
function MesasActivasContent() {
    ...
    const confirmCancel = async (motivo: string) => {
        if (!cancelData || !user || !business) return;
        try {
        ...
        <div className="min-h-screen bg-[#f8fafc] p-4 sm:p-8 lg:p-12 pb-32">
            <div className="max-w-7xl mx-auto">
                ...
                <AnulacionModal
                    ...
                    subtitulo={`Pedido de ${cancelData?.label}`}
                />
            </div>
    ...
    return (
        ...
        >
            ...
            <div className="flex gap-3">
                {(user?.rol === 'admin' || user?.rol === 'cajero' || user?.es_super_admin) && !isReadOnly(user?.rol) && (
                ...
                {onEdit && (user?.rol === 'admin' || user?.es_super_admin || user?.rol === 'cajero') && !isReadOnly(user?.rol) && (
